...
private:
	...
	size_t _GetLeefNode(Node* pRoot)
	{
		...
		return Leftcount + Rightcount;//我在这错了
	}
	...
	void _preorder(Node* pRoot)
	{
		...
		{
			cout << pCur->_data << " ";//访问
			if (!s.empty() && pCur == s.top())//当前访问过的结点如果是栈顶元素就将它出栈
				s.pop();
			if (pCur->_pRight)//保存右孩子
				s.push(pCur->_pRight);

			pCur = pCur->_pLeft;//当前结点移至左孩子

			if (!s.empty() && pCur == NULL )//左孩子为空从栈顶取
				pCur = s.top();
		}
	}
